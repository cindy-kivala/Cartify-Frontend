  ...
  return (
    <div className="p-6 min-h-screen bg-gradient-to-br from-[#0a0a0f] to-[#1a1a2e]">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Grid Layout - Let CSS handle the image sizing */}
          ...
          {products.map((product) => (
            <div key={product.id} className="product-card">
              ...
              <Link to={`/products/${product.id}`} className="hover:text-[#00f0ff] transition">
                <h2 className="product-name mt-2">{product.name}</h2>
              </Link>

              <p className="product-price">{product.price.toFixed(2)}</p>

              {product.category && (
                <span className="bg-[#00f0ff] text-black text-xs px-2 py-1 rounded mt-1 inline-block">
                  {product.category}
                </span>
              )}
...
//           <div
//             key={product.id}
//             className="product-card flex flex-col h-full p-4 rounded-xl hover:shadow-2xl transition"
//           >
//             {/* Image container */}
...
//                 src={product.image_url}
//                 alt={product.name}
//                 className="w-full h-48 object-cover rounded-lg transition-transform duration-300 hover:scale-105 mb-3"
//                 onError={(e) =>
//                   (e.target.src =
